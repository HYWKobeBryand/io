...
quoted_start(const Char* string, Char escape, Char delim)
{
    ...
    quoted_state<Char> state = { string, std::size_t(end - string), count };
    return state;
}
    ...
    std::size_t size, std::size_t count, Char escape, Char delim)
{
    ...
    if (size == count) {
        if ((std::size_t)buf.sputn(string, size) != size) {
            return false;
        }
    } else {
...
template<class Char, class Traits>
inline bool
quoted_fill(std::basic_streambuf<Char, Traits>& buf, Char fill,
    std::size_t width, std::size_t size)
{
    return (width <= size) ||
        boost::io::detail::buffer_fill(buf, fill, width - size);
}
    ...
    Char delim)
{
    ...
    typename std::basic_ostream<Char, Traits>::sentry entry(os);
    if (entry) {
        ...
        if ((os.flags() & std::ios_base::adjustfield) == std::ios_base::left) {
            if (!boost::io::detail::quoted_put(buf, state.string, state.size,
                ...
                !boost::io::detail::quoted_fill(buf, os.fill(),
                    (std::size_t)os.width(), state.count)) {
                return os;
            }
        } else if (!boost::io::detail::quoted_fill(buf, os.fill(),
                (std::size_t)os.width(), state.count) ||
